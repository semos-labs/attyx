...
const TOOLS_RAW =
    ...
    \\{"name":"list_agents","description":"List AI agents running in panes with status and usage. Returns JSON: each agent has pane_id, tab_id, session, pid, state (idle|working|input), message, and a usage object with input_tokens, output_tokens, context_used, context_max (context window size), cost_usd, cost_is_estimate, model, and effort. Unknown usage fields are omitted (absent = unknown, not zero).","inputSchema":{"type":"object","properties":{"pane":{"type":"integer","description":"Restrict to one pane id."},"session":{"type":"integer"}}}},
