...
pub fn ptyReaderThread(ctx: *PtyThreadCtx) void {
    ...
    while (c.attyx_should_quit() == 0) {
        ...
        // AI menu toggle check
        if (@atomicRmw(i32, &terminal.g_toggle_ai_menu, .Xchg, 0, .seq_cst) != 0) {
            if (ctx.overlay_mgr) |mgr| {
                if (mgr.isVisible(.ai_demo)) {
                    ai.cancelAi(ctx);
                } else {
            ...
            if (ctx.overlay_mgr) |mgr| {
                ...
                if (mgr.activateFocused()) |action_id| {
                    switch (action_id) {
                        ...
                        .copy => if (ai.g_ai_review_pending) ai.copyRedactedPayload(ctx) else if (ai.g_ai_fix != null) ai.handleFixCopyAction(ctx) else if (ai.g_ai_generate != null) ai.handleGenerateCopyAction(ctx) else if (ai.g_ai_explain != null) ai.handleExplainCopyAction(ctx) else if (ai.g_ai_rewrite != null) ai.handleRewriteCopyAction(ctx) else ai.handleCopyAction(ctx),
                        .retry => ai.handleRetryAction(ctx),
                        .accept => if (ai.g_ai_review_pending) ai.confirmReviewSend(ctx) else if (ai.g_ai_generate != null) ai.submitGeneratePrompt(ctx) else ai.handleEditAcceptAction(ctx),
                ...
                if (mgr.hitTest(click_col, click_row)) |hit| {
                    ...
                    if (mgr.clickAction(hit)) |action_id| {
                        switch (action_id) {
                            ...
                            .copy => if (ai.g_ai_review_pending) ai.copyRedactedPayload(ctx) else if (ai.g_ai_fix != null) ai.handleFixCopyAction(ctx) else if (ai.g_ai_explain != null) ai.handleExplainCopyAction(ctx) else if (ai.g_ai_rewrite != null) ai.handleRewriteCopyAction(ctx) else ai.handleCopyAction(ctx),
                            .retry => ai.handleRetryAction(ctx),
                            .accept => if (ai.g_ai_review_pending) ai.confirmReviewSend(ctx),
                            .custom_0 => ai.handleRewriteReplaceAction(ctx),
                            else => {},
                        }
                    }
